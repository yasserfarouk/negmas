...
designed from the bottom up to simplify the common tasks involved in
...
- **contract_size** Used to specify some sense of size for contracts.
  ...
  the simulation. We define the contract size here as the welfare (total
...
- force_signing: If true, agents are not asked to sign contracts. They
  are forced to do so. In this case, ``default_signing_delay`` is not
