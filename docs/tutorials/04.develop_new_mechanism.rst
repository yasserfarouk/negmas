...
3. Optionally create a new ``MechanismState`` ``dataclass`` with any
   ...
   the ``state_factory`` argument passed to the ``Mechanism``
...
.. code:: ipython3
    ...
    fig.add_trace(
        go.Scatter(
            x=values, y=a1values, mode="lines", name="First negotiator", showlegend=False
        ),
        row=2,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            x=values, y=a2values, mode="lines", name="Second negotiator", showlegend=False
        ),
        row=2,
        col=1,
    )
...
.. code:: ipython3
    ...
    fig.add_trace(
        go.Scatter(
            x=slopes, y=a1values, mode="lines", name="First negotiator", showlegend=False
        ),
        row=2,
        col=1,
    )
    fig.add_trace(
        go.Scatter(
            x=slopes, y=a2values, mode="lines", name="Second negotiator", showlegend=False
        ),
        row=2,
        col=1,
    )
    ...
    def plot_a_run(mechanism: RubinsteinMechanism) -> None:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=second[:, 1],
                mode="markers",
                marker=dict(symbol="cross", color="blue", size=10),
...
Now we can see when the first negotiator is a ``boulware`` (i.e. it
