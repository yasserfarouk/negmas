...
.. code:: ipython3
    ...
    for strategy in strategies:
        data = results.scores[results.scores["strategy"] == strategy]["advantage"].dropna().values
        if len(data) > 1:
            kde = stats.gaussian_kde(data)
            x_range = np.linspace(data.min() - 0.5, data.max() + 0.5, 200)
            fig.add_trace(go.Scatter(x=x_range, y=kde(x_range), mode='lines', name=str(strategy)))
    fig.update_layout(title="Advantage Distribution by Strategy", xaxis_title="Advantage", yaxis_title="Density")
    fig.show()
