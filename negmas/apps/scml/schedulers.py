...
from typing import Dict, Any, Callable, Collection, List, Optional, Sequence, Type
...
from .common import FactoryStatusUpdate, ProductionNeed, Job, Product, Process, ProductManufacturingInfo, SCMLAgreement, \
    ManufacturingProfile, ManufacturingProfileCompiled
from .simulators import FactorySimulator, SlowFactorySimulator, NO_PRODUCTION
...
class Scheduler(ABC):
    ...
    def __init__(self, manager_id: str, awi: 'SCMLAWI', horizon: Optional[int] = None
                 , max_insurance_premium: Optional[float] = None):
...
class GreedyScheduler(Scheduler):
    ...
    def schedule_contract(self, contract: Contract
                          , end: int = None, ensure_storage_for: int = 0) -> ScheduleInfo:
        """
        ...
        if contract.agreement is None:
            return ScheduleInfo(end=end, final_balance=self.simulator.balance(end - 1))
        ...
        t = agreement['time']
        # @todo add the contract to ignored_contracts if t < simulator.fixed_before
        q, u = int(agreement['quantity']), agreement['unit_price']
        p = u * q
        pid: int = contract.annotation['cfp'].product
        total = simulator.total_storage_to(end-1)
        if contract.annotation['buyer'] == self.manager_id:
            ...
            if (insurance is None or simulator.wallet(t) >= p * (1.0 + insurance)) and \
                (simulator.max_storage is None or np.max(total[t:]) <= simulator.max_storage - q):
                if insurance is not None and insurance > 0.0:
                    simulator.pay(insurance, t=t)
                simulator.buy(product=pid, quantity=q, price=p, t=t)
                return ScheduleInfo(valid=True, end=end, final_balance=self.simulator.balance(end - 1))
            return ScheduleInfo(valid=False, failed_contracts=[contract], end=end
                                , final_balance=self.simulator.balance(end - 1))
        elif contract.annotation['seller'] == self.manager_id:
            # I am a seller

            # if enough is available in storage and not reserved, just sell it
            q_needed = q - simulator.available_storage(t)[pid]
            if q_needed <= 0:
                simulator.sell(product=pid, quantity=q, price=p, t=t)
                return ScheduleInfo(end=end, final_balance=self.simulator.balance(end - 1))
            jobs: List[Job] = []
            ...
            while q_needed > 0 and some_production:
                ...
                for info in self.producing[pid]:
                    ...
                    current_schedule = simulator.line_schedules_to(t - ensure_storage_for - 1)[line]
                    ...
                    if not simulator.schedule(job, override=False):
                    ...
                    for i in process.inputs:
                        ...
                        step = ptime+int(math.floor(i.step * length))
                        available = max(0, self.simulator.available_storage(step)[pind] - quantity)
                        ...
                        if tobuy > 0 or instore > 0:
                            needs.append(ProductionNeed(product=pind, needed_for=contract
                                                        , quantity_in_storage=instore, quantity_to_buy=tobuy, step=t))
            ...
            if q_needed <= 0:
                ...
                schedule = ScheduleInfo(jobs=jobs, end=end, needs=needs, failed_contracts=[]
                                        , final_balance=self.simulator.balance(end - 1))
                simulator.delete_bookmark(bookmark)
                return schedule
