...
utility = 0.17682
accepted = [(0,), (2,)]
ufun = MappingUtilityFunction(dict(zip([(_,) for _ in range(n_outcomes)], [utility] * n_outcomes)), reserved_value=0.0)
...
def master(true_utilities, strategy_name='titration-0.05'):
    user = User(ufun=MappingUtilityFunction(dict(zip([(_,) for _ in range(n_outcomes)]
                                                     , true_utilities)), reserved_value=0.0), cost=cost)
...
class TestCountableOutcomesUser(object):
    ...
    def test_countable_outcmoes_user_can_elicit_bisection(self, neg, user):
        ...
        for i in range(len(elicited) - 1):
            assert elicited[i + 1].scale == 0.5 * elicited[i].scale, 'uncertainty is not decreasing'
        for i in range(len(estimated) - 1):
            assert estimated[i + 1].scale == 0.5 * estimated[i].scale, 'uncertainty is not decreasing'

    def test_countable_outcmoes_user_can_elicit_titration_up(self, neg, user):
        step = 0.05
        strategy = EStrategy(strategy=f'titration+{step}'
                             , resolution=1e-4)
        strategy.on_enter(neg.ami)
        elicited = []
        estimated = []
        ...
        assert elicited[0].scale == 1.0 - step, 'first scale is incorrect in elicitation'
        assert estimated[0].loc == 0.0, 'first loc is incorrect in estimation'
        assert estimated[0].scale == 1.0, 'first scale is incorrect in estimation'
        for u, e in zip(elicited[:-1], estimated[:-1]):
            assert abs(u.scale - e.scale + step) < 1e-3, 'uncertainty is not decreasing as expected'
        for i in range(len(elicited) - 2):
            assert abs(elicited[i + 1].scale - elicited[i].scale + step) < 1e-3, 'uncertainty is not decreasing'
        for i in range(len(estimated) - 2):
            assert (estimated[i + 1].scale - estimated[i].scale + step) < 1e-3, 'uncertainty is not decreasing'

    def test_countable_outcmoes_user_can_elicit_titration_up_no_step(self, neg, user):
        step = 0.01
        strategy = EStrategy(strategy=f'titration'
                             , resolution=step)
        strategy.on_enter(neg.ami)
        elicited = []
        estimated = []
    ...
    def test_countable_outcmoes_user_can_elicit_titration_down(self, neg, user):
        ...
        step = -step
        assert elicited[0].loc + elicited[0].scale == 1.0 - step, 'first loc is incorrect in elicitation'
        assert elicited[0].scale == 1.0 - step, 'first scale is incorrect in elicitation'
        assert estimated[0].loc == 0.0, 'first loc is incorrect in estimation'
        assert estimated[0].scale == 1.0, 'first scale is incorrect in estimation'
    ...
    def test_countable_outcmoes_user_can_elicit_dtitration_up(self, neg, user):
        step = 0.05
        strategy = EStrategy(strategy=f'dtitration+{step}'
                             , resolution=1e-4)
        strategy.on_enter(neg.ami)
        elicited = []
        estimated = []
    ...
    def test_countable_outcomes_user_can_return_all_queries(self, neg):
        for s in ('exact', 'bisection', 'titration+0.05', 'titration-0.5'
                  , 'dtitration+0.5', 'dtitration-0.05', 'pingpong0.5', 'dpingpong0.5'):
            user = User(ufun=ufun, cost=cost)
            strategy = EStrategy(strategy=s)
            strategy.on_enter(neg.ami)
            q = possible_queries(ami=neg.ami, strategy=strategy, user=user)
            assert len(q) > 0 and s != 'exact' or len(q) == 0 and s == 'exact', 'returns some queries'
    ...
    def test_elicit_until(self, neg):
        for s in ('bisection', 'titration+0.05', 'titration-0.05', 'pingpong'):
            user = User(ufun=ufun, cost=cost)
            stretegy = EStrategy(strategy=s
                                 , resolution=1e-3)
            stretegy.on_enter(neg.ami)
            outcome, query, qcost = next_query(strategy=stretegy, outcome=(0,), user=user)[0]
            stretegy.until(user=user, outcome=(0,)
...
def data_folder():
    return pkg_resources.resource_filename(
        "negmas", resource_name="tests/data"
    )
...
class TestCountableOutcomesElicitor(object):
    ...
    def test_dummy(self, master, true_utilities):
        ...
        neg = SAOMechanism(
            ...
            , n_steps=10)
        accepted = [(0,), (2,)]
        opponent = LimitedOutcomesNegotiator(outcomes=n_outcomes,
    ...
    def _run_optimal_test(self, elicitor: Union[str, 'BaseElicitor'], master, true_utilities
                          , **kwargs):
        neg = SAOMechanism(
            outcomes=n_outcomes
            , n_steps=10)
    ...
    def test_pareto_frontier_in_mechanism(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('FullElicitor', master, true_utilities)
    ...
    def test_pareto_frontier_2(self):
        ...
        outcomes = [(_,) for _ in range(n_outcomes)]
        accepted = [(2,), (3,), (4,), (5,)]
        elicitor_utilities = [0.5337723805661662, 0.8532272031479199, 0.4781281413197942, 0.7242899747791032,
                              0.3461879818432919, 0.2608677043479706, 0.9419131964655383, 0.29368079952747694,
                              0.6093201983562316, 0.7066918086398718]
        # list(np.random.rand(n_outcomes).tolist())
        opponent_utilities = [1.0 if (_,) in accepted else 0.0 for _ in range(n_outcomes)]
        frontier, frontier_locs = pareto_frontier([MappingUtilityFunction(lambda o: elicitor_utilities[o[0]]
                                                                          , reserved_value=reserved_value),
                                                   MappingUtilityFunction(lambda o: opponent_utilities[o[0]]
                                                                          , reserved_value=reserved_value)]
                                                  , outcomes=outcomes
                                                  , sort_by_welfare=True)
        welfare = (np.asarray(elicitor_utilities) + np.asarray(opponent_utilities)).tolist()
        ...
        opponent = LimitedOutcomesNegotiator(
            ...
            acceptable_outcomes=accepted,
            acceptance_probabilities=[1.0] * len(accepted))
        eufun = MappingUtilityFunction(dict(zip(outcomes, elicitor_utilities)), reserved_value=reserved_value)
    ...
    def test_random(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('RandomElicitor', master, true_utilities)

    def test_optimal(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('PandoraElicitor', master, true_utilities)

    def test_fast(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('FastElicitor', master, true_utilities)

    def test_balanced(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('BalancedElicitor', master, true_utilities)

    def test_mean(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('MeanElicitor', master, true_utilities)

    def test_optimistic(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('OptimisticElicitor', master, true_utilities)

    def test_pessimistic(self, master, true_utilities):
        neg, elicitor, opponent = self._run_optimal_test('PessimisticElicitor', master, true_utilities)
    ...
    def test_loading_laptop(self, data_folder):
        domain, agents_info, issues = load_genius_domain_from_folder(os.path.join(data_folder, "Laptop")
                                                                     , force_single_issue=True
                                                                     , keep_issue_names=True
                                                                     , keep_value_names=True
                                                                     ,
                                                                     agent_factories=lambda: LimitedOutcomesNegotiator(
                                                                         outcomes=n_outcomes)
                                                                     )
        ...
        assert front == [(0.7715533992081258, 0.8450562871935449), (0.5775524426410947, 1.0), (1.0, 0.5136317604069089),
...
def test_elicitation_run_with_no_conflict():
    n_outcomes = 100
    n_steps = 100
    config = SAOElicitingMechanism.generate_config(cost=0.05, n_outcomes=n_outcomes,
                                                   conflict=0.0
                                                   , winwin=1.0
                                                   , n_steps=n_steps,
...
def test_alternating_offers_eliciting_mechanism():
    for strategy, dynamic in [('pingpong', False), ('bisection', True), ]:
        config = SAOElicitingMechanism.generate_config(
            cost=0.001, n_outcomes=10, opponent_model_adaptive=False
            , opponent_type='limited_outcomes', conflict=1.0, n_steps=500, time_limit=100000.0
            , own_utility_uncertainty=0.1
            , own_reserved_value=0.1,
        )
        p = SAOElicitingMechanism(**config, elicitation_strategy=strategy, elicitor_type='balanced'
...
def test_alternating_offers_eliciting_mechanism_voi():
    for strategy, dynamic in [(None, False), (None, True), ]:
        ...
        p = SAOElicitingMechanism(**config, elicitation_strategy=strategy, elicitor_type='voi'
                                  , dynamic_queries=dynamic)
        p.run()
        assert len(p.history) > 0
        #pprint(p.elicitation_state)
...
def test_alternating_offers_eliciting_mechanism_voi_optimal():
    ...
    # print(p.agents[1].reserved_value)
    assert p.elicitation_state['elicitation_cost'] >= 0.0
    assert p.elicitation_state['elicitor_utility'] + p.elicitation_state['elicitation_cost'] >= p.negotiators[1].reserved_value
    assert p.elicitation_state['total_voi'] is not None and p.elicitation_state['total_voi'] > 0
...
def test_alternating_offers_eliciting_mechanism_full_knowledge():
    ...
    p = SAOElicitingMechanism(**config, elicitation_strategy='bisection'
                              , elicitor_type='full_knowledge')
    p.run()
    assert len(p.history) > 0
    #pprint(p.elicitation_state)
    # print(p.agents[1].reserved_value)
...
def test_a_small_elicitation_session():
    import os
    utilities = [U('uniform', loc=0.0, scale=1.0),
