...
from hypothesis._strategies import integers, booleans
from pytest import fixture, mark

from negmas.apps.scml import ManufacturingProfile, Product, Process, InputOutput, Job, RunningCommandInfo
from negmas.apps.scml.simulators import SlowFactorySimulator, NO_PRODUCTION
...
@given(integers(min_value=0, max_value=n_steps * 2))
def test_slow_factory_simulator_can_be_checked_at_any_time(slow_simulator, t):
    assert slow_simulator.wallet(t) == initial_wallet
    assert slow_simulator.total_storage(t) == sum(initial_storage.values())
...
def do_simulator_run(slow_simulator, profiles, t, at, profile_ind, override):
    ...
    assert slow_simulator.wallet(at) == (initial_wallet if at < t else initial_wallet - cost)
    assert slow_simulator.total_storage(at) == sum(initial_storage.values()) - total_inputs + total_outputs
    assert (slow_simulator.line_schedules(at)[_] == NO_PRODUCTION for _ in range(n_lines) if _ != line)
    if at < t or at >= t + length:
        assert slow_simulator.line_schedules(at)[line] == NO_PRODUCTION


@mark.parametrize('profile_ind,t,at_,override',
                  [
                      (0, 10, 'after', True), (0, 10, 'after', False), (0, 10, 'before', True), (0, 10, 'before', False)
                      , (0, 10, 'just before', True), (0, 10, 'just before', False), (0, 10, 'just after', True)
                      , (0, 10, 'just after', False)
                      , (0, 10, 'at', True), (0, 10, 'at', False), (0, 10, 'middle', True), (0, 10, 'middle', False)
                      , (1, 10, 'after', True), (1, 10, 'after', False), (1, 10, 'before', True)
                      , (1, 10, 'before', False)
                      , (1, 10, 'just before', True), (1, 10, 'just before', False), (1, 10, 'just after', True)
                      , (1, 10, 'just after', False)
                      , (1, 10, 'at', True), (1, 10, 'at', False), (1, 10, 'middle', True), (1, 10, 'middle', False)
                      , (2, 10, 'after', True), (2, 10, 'after', False), (2, 10, 'before', True)
                      , (2, 10, 'before', False)
                      , (2, 10, 'just before', True), (2, 10, 'just before', False), (2, 10, 'just after', True)
                      , (2, 10, 'just after', False)
                      , (2, 10, 'at', True), (2, 10, 'at', False), (2, 10, 'middle', True), (2, 10, 'middle', False)
                      , (3, 10, 'after', True), (3, 10, 'after', False), (3, 10, 'before', True)
                      , (3, 10, 'before', False)
                      , (3, 10, 'just before', True), (3, 10, 'just before', False), (3, 10, 'just after', True)
                      , (3, 10, 'just after', False)
                      , (3, 10, 'at', True), (3, 10, 'at', False), (3, 10, 'middle', True), (3, 10, 'middle', False)
                      , (0, 0, 'after', True), (0, 0, 'after', False), (0, 0, 'before', True), (0, 0, 'before', False)
                      , (0, 0, 'just before', True), (0, 0, 'just before', False), (0, 0, 'just after', True)
                      , (0, 0, 'just after', False)
                      , (0, 0, 'at', True), (0, 0, 'at', False), (0, 0, 'middle', True), (0, 0, 'middle', False)
                      , (1, 0, 'after', True), (1, 0, 'after', False), (1, 0, 'before', True)
                      , (1, 0, 'before', False)
                      , (1, 0, 'just before', True), (1, 0, 'just before', False), (1, 0, 'just after', True)
                      , (1, 0, 'just after', False)
                      , (1, 0, 'at', True), (1, 0, 'at', False), (1, 0, 'middle', True), (1, 0, 'middle', False)
                      , (2, 0, 'after', True), (2, 0, 'after', False), (2, 0, 'before', True)
                      , (2, 0, 'before', False)
                      , (2, 0, 'just before', True), (2, 0, 'just before', False), (2, 0, 'just after', True)
                      , (2, 0, 'just after', False)
                      , (2, 0, 'at', True), (2, 0, 'at', False), (2, 0, 'middle', True), (2, 0, 'middle', False)
                      , (3, 0, 'after', True), (3, 0, 'after', False), (3, 0, 'before', True)
                      , (3, 0, 'before', False)
                      , (3, 0, 'just before', True), (3, 0, 'just before', False), (3, 0, 'just after', True)
                      , (3, 0, 'just after', False)
                      , (3, 0, 'at', True), (3, 0, 'at', False), (3, 0, 'middle', True), (3, 0, 'middle', False)
                  ])
def test_slow_factory_simulator_with_jobs(slow_simulator, profile_ind, t, at_, override):
    profiles = slow_simulator._factory.profiles
    profile = profiles[profile_ind]
    length = profile.n_steps
    if at_ == 'before':
        if t - 5 < 0:
    ...
    do_simulator_run(slow_simulator, profiles, t, at, profile_ind, override)


@given(profile_ind=integers(min_value=0, max_value=3), t=integers(min_value=0, max_value=n_steps),
       at=integers(min_value=0, max_value=n_steps + 1), override=booleans())
def test_slow_factory_simulator_with_jobs_hypothesis(profile_ind, t, at, override):
    ...
    slow_simulator = SlowFactorySimulator(initial_wallet=initial_wallet, initial_storage=initial_storage
                                          , n_steps=n_steps, n_products=len(products), profiles=profiles
                                          , max_storage=max_storage)
    do_simulator_run(slow_simulator, profiles, t, at, profile_ind, override)
