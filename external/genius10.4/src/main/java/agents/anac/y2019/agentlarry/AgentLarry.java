...
public class AgentLarry extends AbstractNegotiationParty {
    ...
    public Action chooseAction(List<Class<? extends Action>> list) {
        try {
            ...
            LogisticRegression larryModel = this.initializeLarryModel();

            System.out.println("searching for best bid");
            Bid nextBid = this.findNextBid(logisticRegressionsModels, larryModel);
    ...
    private boolean shouldAccept(Bid nextBid) {
        if (lastOfferedBid != null) {
            if (userModel.getBidRanking().getBidOrder().indexOf(lastOfferedBid) >=
                    userModel.getBidRanking().getBidOrder().indexOf(nextBid) * this.utilitySpace.getDiscountFactor()) {
                return true;
            }
        }
        return false;
     ...
     */
    private Bid findNextBid(List<LogisticRegression> logisticRegressionsModels, LogisticRegression larryModel) {
        ...
        while (bidIterator.hasNext()) {
            ...
            double bidUtility = larryModel.classify(vector);
    ...
    private List<LogisticRegression> initializeModels() {
        ...
        for (BidHistory bidHistory : this.agentsBidHistories.values()) {
            ...
            for (Tuple<Bid, Boolean> bidToDidAccept : bidHistory) {
                ...
                double label = bidToDidAccept.get2() ? 1 : 0;
    ...
    private LogisticRegression initializeLarryModel() {
        LogisticRegression model = new LogisticRegression(this.vectorConverter.getVectorSize(this.utilitySpace.getDomain()));
        List<Bid> bids = userModel.getBidRanking().getBidOrder();
        for (int j = 0; j < 5; j++) {
            for (int i = 0; i < bids.size(); i++) {
                Vector vector = this.vectorConverter.convert(bids.get(i));
                double label = (userModel.getBidRanking().getHighUtility() - userModel.getBidRanking().getLowUtility()) * i / bids.size();
