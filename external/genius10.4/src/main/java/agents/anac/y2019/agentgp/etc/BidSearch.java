...
public class BidSearch
{
	...
	public BidSearch(AbstractUtilitySpace utilitySpace, NegotiationInfo negotiationInfo, boolean isPrinting) throws Exception {
		...
		if(this.isPrinting){ System.out.println("BidSearch:success"); }
	}
	...
	private void initMaxBid() throws Exception{
		...
		for (int i = 0; i < tryNum; i++) {
			try {
				do{ SimulatedAnnealingSearch(maxBid, 1.0);
	...
	//	ArrayList<Issue> issues=(ArrayList<Issue>) utilitySpace.getDomain().getIssues(); //changed
...
//					nextBid.setValue(issue.getNumber(), NegotiationInfo.getValuebyFrequencyList(sender, issue)); // 頻出valueを優先してセットする
...
//			}
	...
	private Bid getConvertBidbyOrdinalScale(Bid baseBid) {
		...
		for(Issue issue: negotiationInfo.getIssues()){
			//バグるかも
			//ArrayList<Tuple<Value, Integer>> randomOrderIssues = new ArrayList<>(issues.get(issue)); //changed
			//Collections.shuffle(randomOrderIssues); // ランダムに並び替える
