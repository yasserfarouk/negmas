	...
	public Action chooseAction() {
		...
		try {
			...
			if (myLastBid == null || oponnetLastBid == null) {
				...
				double myUtility = this.utilitySpace
						.getUtilityWithDiscount(myLastBid, time);
				...
				if (opponentUtility >= myUtility * 0.99
						...
						|| bidChooser
								.getMyBidsMinUtility(time) <= opponentUtility) {
				...
				// most optimal bids
					...
					if (time >= 0.9995) {
						...
						if (utilitySpace.getUtilityWithDiscount(myLastBid,
								time) < utilitySpace
