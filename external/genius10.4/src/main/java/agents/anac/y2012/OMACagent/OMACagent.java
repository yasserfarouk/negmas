...
public class OMACagent extends Agent {
	...
	public double timeInt = 1.0 / intervals;
	...
	private Bid getFinalBid() {
		...
		if (discount >= discountThreshold) {
			...
			if (cTime <= discount / splitFactor) {
				...
				return bid;
			} else if (cTime > 0.01 * (tCount
					+ (int) Math.floor(discount / splitFactor * 100))) {
				nextUti = getNextUti();
				tCount++;
			}
		}
		...
		if (nextUti == -3.0) {
			...
			val = est_mu
					+ (cTime - est_mt) * (est_u - est_mu) / (est_t - est_mt);
		} else if (nextUti == -1.0) {
			val = getOriU(cTime);
		}
	...
	private double getPre() {
		...
		exma = ma[lenA - 1] + avg(res)
				+ std(res) * (1.0 - Math.pow(maxList[lenA - 1], 4))
	...
	public static double std(double[] arr) {
		...
		std = (len / (len - 1.0)) * (ssum / len - Math.pow(avg(arr), 2));
