...
public class UtilitySpaceHypothesis extends Hypothesis
{
  ...
  public UtilitySpaceHypothesis(Domain pDomain, AdditiveUtilitySpace pUS, WeightHypothesis pWeightHyp, EvaluatorHypothesis[] pEvalHyp)
  {
    fUS = pUS;
    fDomain = pDomain;
  ...
  public double getUtility(Bid pBid) {
    ...
    for (int k = 0; k < fEvalHyp.length; k++) {
      try
      {
        u = u + fWeightHyp.getWeight(k) * 
          fEvalHyp[k].getEvaluator().getEvaluation(fUS, pBid, 
          ((Issue)issues.get(k)).getNumber()).doubleValue();
      }
      catch (Exception e) {
        u = 0.0D;
      }
