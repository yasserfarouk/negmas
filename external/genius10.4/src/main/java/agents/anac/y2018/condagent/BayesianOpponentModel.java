  ...
  extends OpponentModel
{
  ...
  public BayesianOpponentModel(AdditiveUtilitySpace pUtilitySpace) {
    ...
    for (int i = 0; i < issues.size(); i++) {
      P[i] = 
        ((i + 1) / (issues.size() * (fDomain.getIssues().size() + 1) / 2.0D));
    }
    ...
    for (int i = 0; i < fUS.getNrOfEvaluators(); i++) {
      ...
      case OBJECTIVE: 
        ...
        if (USE_DOMAIN_KNOWLEDGE)
        {
          ...
          lHypEval.addParam(1, 
            1.0D / (lHypEval.getUpperBound() - lHypEval
            .getLowerBound()));
          lHypEval.addParam(
            0, 
        ...
        for (int k = 0; k < lEvalHyps.size(); k++) {
          ((EvaluatorHypothesis)lEvalHyps.get(k)).setProbability(
            1.0D / lEvalHyps.size());
        }
      ...
      case DISCRETE: 
        ...
        IssueDiscrete lDiscIssue = 
          (IssueDiscrete)fDomain.getIssues().get(i);
        if (USE_DOMAIN_KNOWLEDGE)
        {
        ...
        else
        {
          ...
          {
            lDiscreteEval.addEvaluation(lDiscIssue.getValue(j), 
              Integer.valueOf(1000 * (lDiscIssue.getNumberOfValues() - 
              j - 1) + 1)); }
    ...
    for (int i = 0; i < fWeightHyps.length; i++)
    {
      ...
      for (int j = 0; j < fEvalHyps.size(); j++) {
        UtilitySpaceHypothesis lUSHyp = new UtilitySpaceHypothesis(
          fDomain, fUS, fWeightHyps[i], (EvaluatorHypothesis[])fEvalHyps.get(j));
        fUSHyps.add(lUSHyp);
      }
    }
  ...
  private double conditionalDistribution(double pUtility, double pPreviousBidUtility)
  {
    ...
    double lResult = 1.0D / (SIGMA * Math.sqrt(6.283185307179586D)) * 
      Math.exp(-(x * x) / (2.0D * SIGMA * SIGMA));
    return lResult;
  }
  ...
  public void updateBeliefs(Bid pBid) throws Exception
  {
    ...
    for (int i = 0; i < fUSHyps.size(); i++) {
      ...
      double condDistrib = hyp.getProbability() * 
        conditionalDistribution(((UtilitySpaceHypothesis)fUSHyps.get(i)).getUtility(pBid), 
        fPreviousBidUtility);
    ...
    for (int i = 0; i < fUSHyps.size(); i++) {
      ...
      if ((fUseMostProbableHypsOnly) && 
        (normalizedProbability > lMaxProb * 0.99D / lFullProb)) {
  ...
  private void buildEvaluationHypsRecursive(ArrayList<EvaluatorHypothesis[]> pHyps, EvaluatorHypothesis[] pEval, int m)
  {
    if (m == 0) {
      ArrayList<EvaluatorHypothesis> lEvalHyps = (ArrayList)fEvaluatorHyps.get(fUS
        .getNrOfEvaluators() - 1);
