...
public class BidSearch {
	...
	public BidSearch(NegotiationInfo info, boolean isPrinting, NegoStats negoStats, NegoHistory negoHistory)
			throws Exception {
		this.info = info;
	...
	private Bid getBidbyAppropriateSearch(Bid baseBid, double threshold) {
		...
		try {
			...
			System.out.println(
					"[Exception_Search] Problem with received bid(SA:last):" + e.getMessage() + ". cancelling bidding");
		}
		return bid;
	}
	...
	private void initMaxBid() throws Exception {
		...
		System.out
				.println("[isPrinting_Search]:" + maxBid.toString() + " " + info.getUtilitySpace().getUtility(maxBid));
	}
