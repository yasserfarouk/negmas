...
public class NegoHistory {
	 ...
	 * @param isPrinting
	 * @param pData
	 *            public PersistentDataContainer getData(): persistent data
	 */
	public NegoHistory(NegotiationInfo info, boolean isPrinting, PersistentDataContainer pData) {
		...
		if (pData.getPersistentDataType() == PersistentDataType.STANDARD) {
			...
			if (!history.isEmpty()) {
				// example of using the history. Compute for each party the
				// maximum
				...
				System.out.println(maxutils); // notice tournament suppresses
												// all
				// output.
			}
		}
