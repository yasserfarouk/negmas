...
public class negotiationStrategy {
	...
	static private double PF = 0.5; // 最終提案フェーズにおいて互いが妥協戦略を選択した場合に，自身が相手よりも先に譲歩する確率
	...
	public double getThreshold(double time) {
		...
		if (utilitySpace.discount(1.0, time) == 1.0) {
			...
			threshold = Math.max(1.0 - time / utilitySpace.discount(1.0, time), target);
		}

		// デバッグ用
		if (isPrinting) {
			System.out.println("threshold = " + threshold + ", opponents:" + negotiationInfo.getOpponents());
		}
