...
import agents.anac.y2016.farma.etc.CntBySender;
...
public class negotiationInfo {
	...
	private HashMap<Value, CntBySender> offeredValueNum;
	...
	public negotiationInfo(AbstractUtilitySpace utilitySpace, boolean isPrinting) {
		...
		System.out.println(offeredValueNum);
		try {
	...
	public Value getHighFrequencyValue(Object sender, Issue issue) {
		ArrayList values = this.getValues(issue);
		double tempMax = 0.0D;
		Value tempValue = (Value) values.get(0);
		Iterator var8 = values.iterator();

		while (var8.hasNext()) {
			Value value = (Value) var8.next();
			if (this.offeredValueNum.containsKey(value)) {
				double nowCnt = (double) ((CntBySender) this.offeredValueNum.get(value)).getSimpleCnt(sender);
				if (tempMax < nowCnt) {
					tempMax = nowCnt;
					tempValue = value;
				}
	...
	public void updateOfferedValueNum(Object sender, Bid bid, double time) {
		Value value;
		for (Iterator var6 = this.issues.iterator(); var6.hasNext(); ((CntBySender) this.offeredValueNum.get(value))
				.incrementCnt(sender, time)) {
			Issue issue = (Issue) var6.next();
			value = bid.getValue(issue.getNumber());
			if (!this.offeredValueNum.containsKey(value)) {
				this.offeredValueNum.put(value, new CntBySender(0));
			}
		}

	}

	// 入力されたbidがそれぞれのsender でどれほど頻度的に多いのかチェック
	public int getBidFrequcncy(Bid aBid) {
		int tPoint = 0;
		for (Map.Entry<Integer, Value> e : aBid.getValues().entrySet()) {
			if (this.offeredValueNum.containsKey(e.getValue())) {
				tPoint += offeredValueNum.get(e.getValue()).getSimpleSum();
			}
		}
		return tPoint;
	}
