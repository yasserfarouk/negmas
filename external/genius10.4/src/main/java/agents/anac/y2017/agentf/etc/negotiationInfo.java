...
public class negotiationInfo {
		...
		for (Issue issue : issues) {
			...
			for (Value value : values) {
				...
				valueRelativeUtility.get(issue).put(value,
						utilitySpace.getUtility(currentBid) - utilitySpace.getUtility(maxBid));
			}
		}
	}
	...
	public void updateNegotiatingInfo(Object sender, Bid offeredBid) throws Exception {
		...
		opponentsVariance.put(sender,
				(opponentsPowSum.get(sender) / round_num) - Math.pow(opponentsAverage.get(sender), 2)); // 分散
	...
	private void updateFrequencyList(Object sender, Bid offeredBid) throws Exception {
		for (Issue issue : issues) {
			...
			opponentsValueFrequency.get(sender).get(issue).put(value,
					opponentsValueFrequency.get(sender).get(issue).get(value) + 1); // リストを更新
		...
		for (Issue issue : issues) {
			...
			for (Tuple<Value, Integer> tuple : opponentsValueOrdinalScale.get(sender).get(issue)) {
				if (tuple.get1().equals(value)) {
					opponentsValueOrdinalScale.get(sender).get(issue).set(i,
							new Tuple<Value, Integer>(value, tuple.get2() + 1));
					break;
				}
				i++;
			}
	...
	public void sortOrdinalScale() throws Exception {
		for (Object opponent : opponents) {
			for (Issue issue : issues) {
				Tuple<Value, Integer> tuplePre = opponentsValueOrdinalScale.get(opponent).get(issue)
						.get(opponentsValueOrdinalScale.get(opponent).get(issue).size() - 1);
				for (int i = opponentsValueOrdinalScale.get(opponent).get(issue).size() - 2; i >= 0; i--) {
					if (opponentsValueOrdinalScale.get(opponent).get(issue).get(i).get2() < tuplePre.get2()) {
						opponentsValueOrdinalScale.get(opponent).get(issue).set(i + 1,
								opponentsValueOrdinalScale.get(opponent).get(issue).get(i));
	...
	public void updatePBList(Bid popularBid) throws Exception {
		if (!PBList.contains(popularBid)) { // 一意に記録
			...
			if (isPrinting) {
				...
				System.out
						.println("Size:" + PBList.size() + ", Min:" + utilitySpace.getUtility(PBList.get(0)) + ", Max:"
								+ utilitySpace.getUtility(PBList.get(PBList.size() - 1)) + ", Opponents:" + opponents);
			}
		}
	}
	...
	// 相手の提案履歴の頻度を返す
	public HashMap<Issue, HashMap<Value, Integer>> getHistory(Object sender) {
	...
	// opponentsValueFrequency.get(opponent).get(issue);
	// // List 生成 (ソート用)
	// List<Map.Entry<Value, Integer>> entries = new ArrayList<Map.Entry<Value,
	// Integer>>(
	// values.entrySet());
	// Collections.sort(entries, new Comparator<Map.Entry<Value, Integer>>() {
	// @Override
	// public int compare(Entry<Value, Integer> entry1, Entry<Value, Integer>
	// entry2) {
	// return ((Integer) entry2.getValue()).compareTo((Integer)
	// entry1.getValue());
	// }
	// });
	//
	...
	// // ArrayList<Tuple<Value, Integer>> values =
	// // opponentsValueOrdinalScale.get(opponent).get(issue);
	// // 内容を表示
