...
1.  An agent requests a negotiation involving a set of agents (it may or
    ...
    negotition is complete. The ``World`` can disble immediately running
    negotiations, or requesting negotiations that does not involve
    oneself, etc. The caller agent can also provide a ``Negotiator`` to
    represent it in this negotiation.
...
statistics by overloading ``post_step_stats`` (and the coresponding
...
un-necessary details from developers of agents targeting our
...
to a negotiatio request from the world in
...
-  There is also an **on_contrats_finalized** callback that is used to
   tell the agent about which of its agreements have been signed by
   everyone and became binding and which were cancelled because one or
...
works. The most important of thesea are ``name`` to set a name for the
...
-  mechanisms: The mechanism types allowed in this world associated with
   each keyward arguments to be passed to it. This is a dictionary
...
stored by defalut but you can enable thme and control their frequency
...
agents for your newly dreated world.
