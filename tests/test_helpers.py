...
def test_config_reader():
    ...
    test, children = A.from_config(config={'a': 10, 'b': [1, 2, 3]}
                                   , ignore_children=False, scope=locals())
...
def test_config_reader_with_enum():
    ...
    test, children = A.from_config(config={'a': 10, 'b': [1, 2, 3], 'e:E': 1}
                                   , ignore_children=False, scope=locals())
    assert isinstance(test, A)
    assert test.a == 10
    assert test.b == [1, 2, 3]
